...
import {
  Container, Title, TextInput, Button, Card, Image, Text,
  Group, Loader, Alert, Stack, Badge
} from '@mantine/core'
...
export function CreatePage() {
  ...
  return (
    <Container size="md" py="xl">
      ...
      <Title order={2} mb="xl">Create a new MusicLink</Title>
      ...
      {searchError && <Alert color="red" mb="md">{searchError}</Alert>}

      {creating && (
        <Group justify="center" py="xl">
          <Loader /><Text>Creating MusicLink...</Text>
        </Group>
      )}

      <Stack gap="sm">
        {results.map((track) => (
          <Card key={track.id} withBorder padding="sm" style={{ cursor: 'pointer' }}
